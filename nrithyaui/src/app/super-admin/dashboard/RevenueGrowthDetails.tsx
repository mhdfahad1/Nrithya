...
type RevenueGrowthDetailsProps = {
  from: string;
  to: string;
};

function RevenueGrowthDetails({ from, to }: RevenueGrowthDetailsProps) {
  const {
    ...
    <div>
      <Dialog>
        ...
        <DialogContent className="max-w-[60%] max-h-[90%] overflow-y-auto">
          <DialogHeader>
            <DialogTitle className="text-sm">Revenue Growth Detail</DialogTitle>
          </DialogHeader>
          <div>
            {isLoading ? (
              ...
              <Card className="p-4 min-w-[100%]">
                <CardContent>
                  ...
                  ) : RevenueGrowthData !== undefined ? (
                    <DataTable columns={columns} data={RevenueGrowthData} />
                  ) : (
                    "No data to show"
                  )}{" "}
                </CardContent>
